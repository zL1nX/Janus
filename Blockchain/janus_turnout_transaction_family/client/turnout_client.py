...
class TurnoutType(Enum):
    DEVICE_ON_CHAIN = 0x11
    DEVICE_OFF_CHAIN = 0x12
    DEVICE_BOTH = 0x13
    ATTEST_ON_CHAIN = 0x21
    ATTEST_OFF_CHAIN = 0x22
...
class TurnoutClient(object):
    ...
    def submit_condition(self, condition, device_id):
        storageAddress = _assembleAddress(device_id + "condition")
        LOGGER.info('Storage Address %s.',
                storageAddress)
        address_list = [storageAddress] # only need this
        return self._wrap_and_send("change_device_condition", condition, address_list, address_list, wait=10)
    
    def submit_attestation_state(self, att_state, device_id):
        storageAddress = _assembleAddress(device_id + "state")
        LOGGER.info('Storage Address %s.',
                storageAddress)
        address_list = [storageAddress] # only need this
        result = self._wrap_and_send("set_attestation_state", att_state, address_list, address_list, wait=10)
        return result
