...
uint64_t convert_to_uint64_t(const uint8_t* input) {
    uint64_t output = 0;
    for (int i = 0; i < 8; i++) {
...
int janus_puf_evaluate(serial_t* sr, uint8_t* response, const uint8_t* challenge)
{
    ...
    for (int i = 0; i < IPUF_CHALL; i++) {
        ...
        // aprint(shifted, 8, "chall");
        if(puf_evaluate(sr, response + i * 2, shifted) == EXIT_FAILURE)
        {
...
int test_main()
{
    ...
    uint8_t challenge[IPUF_CHALL] = {1,2,3,4,5,6,7,8}, response[PUF_RESPONESE_LEN];
